...
def run():
    ...
    history = model.fit(x_train, y_train, epochs=10, batch_size=1280, shuffle=True, validation_split=0.1)  # 训练
    end_time = time.time()
    print("花费时间为{}".format(end_time - start_time))

    model.save('./model/DGA_predict_BLSTM_V8.h5')
...
def predict(data):
    ...
    model = load_model('./model/DGA_predict_BLSTM_V7.h5')
    start_time = time.time()
